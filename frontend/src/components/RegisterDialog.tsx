...
import { registerUser } from "@/util/api-request/UserUtil";
...
export default function RegisterDialog({
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    serverRegister(email,name,password);
  };
  ...
  const serverRegister = async (email:string,name:string,password:string) => {
    try {
      ...
      apiErrorHandle(error as APIError, () => {});
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog isOpen={isOpen} onClose={handleClose} title={"Register to Blogify"} outsideClickCloses={false}>
      <EmailVerification
        email={email}
