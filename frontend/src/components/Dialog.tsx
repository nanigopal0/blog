...
export default function Dialog({
  ...
  const handleBackdropClick = (event: React.MouseEvent<HTMLDivElement>) => {
    if (outsideClickCloses && event.target === event.currentTarget) {
      onClose();
    }
  };
  ...
  return createPortal(
    ...
    >
      <div
        className="w-full max-h-[90vh] max-w-md lg:max-w-xl bg-white 
      ...
      >
        ...
        {/* Content */}
        <div className="p-4 overflow-y-auto dark:text-white">{children}</div>
      </div>
    </div>,
    document.body,
  );
