...
interface PaginationRoundedProps extends Page {
...
export default function PaginationRounded({
  ...
  const getPageNumbers = () => {
    ...
    let startPage = Math.max(1, number - Math.floor(maxVisiblePages / 2));
  ...
  const handlePageChange = (newPage:number) => {
    if (newPage >= 1 && newPage <= totalPages && newPage !== number) {
    ...
    <div className="max-w-sm sm:max-w-lg w-full flex flex-col sm:flex-row items-center justify-between gap-4 my-4 p-4 bg-white/50 border border-black/20 dark:bg-white/20 rounded-lg shadow-lg">
      ...
      <div className="flex items-center gap-2">
        ...
        <select
          id="page-size-select"
          value={size}
          onChange={(e) => onChangePage(number - 1, parseInt(e.target.value))}
