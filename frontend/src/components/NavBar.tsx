...
export default function NavBar() {
  ...
  const isActive = (path: string) => location.pathname === path; // Check if the tab is active
  ...
  return (
    <nav className="w-full px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center dark:bg-gray-900/80 backdrop-blur-sm shadow-sm dark:shadow-gray-800  sticky top-0 z-40">
      <p
...
export function ToggleThemeIcon({ theme }: { theme: ThemeContextType }) {
  return (
    <button
      className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
...
export function LoginBtn({ openLoginDialog }: { openLoginDialog: () => void }) {
  return (
    <button
