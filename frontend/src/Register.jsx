...
function Register({ onLogin }) {
  ...
  const fullNameValidate = () => {
    const fullNameRegex = /^[A-Za-z]{2,}(?:[-'][A-Za-z]+)?\s[A-Za-z]{2,}(?:[-'][A-Za-z]+)?(?:\s[A-Za-z]{2,}(?:[-'][A-Za-z]+)?)*$/;
    ...
    <div className="h-screen  bg-cover bg-center w-full" style={{ backgroundImage: "url('src/assets/login.jpg')" }}>
      ...
      <div className="flex justify-center items-center h-full  lg:me-10">
        <div className="bg-purple-200 bg-opacity-70 border-2 border-white rounded-xl lg:w-2/5 lg:h-5/6 sm:w-4/6 p-5">
          ...
          <input
            type="text" value={fullName} onChange={(fn) => setFullName(fn.target.value)}
          ...
          <input
            type="email" value={email} onChange={(mail) => setEmail(mail.target.value)}
            className="p-2 w-full border-solid border-2 border-black 
                rounded-xl "
            placeholder="Email"
          />
          ...
          <input
            type="password" value={password} onChange={(pass) => setPassword(pass.target.value)}
          ...
          <input
            type="password" value={confirmPassword} onChange={(confirmPass) => setConfirmPassword(confirmPass.target.value)}
            className="p-2 w-full border-solid border-2 border-black 
                rounded-xl "
          ...
          <div className="w-full my-5 ">
            <button disabled={isRegisterDisabled()} className="disabled:opacity-40 disabled:cursor-not-allowed bg-blue-700 w-full font-medium text-white rounded-md py-1" onClick={() => handleRegister()}>
              Register
            </button>
          </div>
